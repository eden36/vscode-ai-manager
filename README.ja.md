...
- 同じ VS Code エディションのすべての Profile で、チャンネル、完全なモデルカタログ、更新状態、Chat バインディングを共有し、`Settings Sync` で暗号化してほかのデバイスへ同期できます。
...
カスタムチャンネルでは、4 つのプロトコルごとにエンドポイントを設定し、Bearer、`x-api-key`、`x-goog-api-key` のいずれかで認証できます。Gemini のエンドポイントには `{model}` プレースホルダーが必要です。

各モデルのプロトコルは、チャンネルカタログ内の明示的なフィールド、`models.dev` のプロトコルメタデータ、既知のモデルファミリーに対するローカルルール、チャンネルの既定プロトコルの順に判定されます。選択したプロトコルのエンドポイントが存在せずリクエストが失敗した場合、AI Manager は設定済みの別のプロトコルで一度だけ再試行し、その結果を記憶します。

モデルエディターでは、プロトコルと Token 上限を手動で変更できます。ツール呼び出し機能はモデルカタログに従います。カタログが明示している場合はその内容を報告し、明示がない場合は利用可能とみなします。

## デバイス間同期

同じ VS Code エディションのすべての Profile は、ローカル共有ディレクトリを通じて、チャンネル、完全なモデルカタログ、更新結果、Chat バインディング、暗号化 Vault を共有します。いずれかの Profile で Chat 設定を手動変更すると、ほかの Profile にも自動適用されます。Stable、Insiders など異なるエディションのデータは分離されます。

デバイス間同期を有効にすると、共有状態全体が圧縮および整合性検証されてから VS Code の `Settings Sync` に保存されます。API Key は PBKDF2-SHA256 と AES-256-GCM で暗号化されます。マスターパスワードは保存されず、各 Profile の `SecretStorage` には派生キーだけが保存されます。

別のコンピューターでは、同じ VS Code アカウントでサインインして `Settings Sync` を有効にし、AI Manager を使用する各 Profile で同じマスターパスワードを一度入力します。パスワードを紛失した場合は Vault のリセットが必要です。同期された暗号文と API Key は削除されますが、ローカルの非機密設定は保持されます。
...
- 拡張機能はローカルの共有状態ディレクトリを読み書きするため、UI 拡張機能として宣言されています。Remote-SSH、WSL、Codespaces のウィンドウでは、リモートホストではなくローカルマシン上で動作します。
...
モデルカタログの更新時には、各モデルが使用するプロトコルを判定するために `https://models.dev/api.json` にもリクエストします。このリクエストに API Key やユーザーデータは含まれず、結果は 6 時間キャッシュされます。
