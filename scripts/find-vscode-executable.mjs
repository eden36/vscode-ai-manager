...
export function findVSCodeExecutable() {
  loadDebugEnv();
  const configured = process.env.AI_MANAGER_VSCODE_EXECUTABLE?.trim();
  if (configured) {
    if (!fs.existsSync(configured)) throw new Error(`找不到 VS Code 可执行文件：${configured}`);
    return configured;
  }
  const discovered = discoverFromPath();
  if (!discovered) throw new Error('找不到 VS Code。请安装 VS Code 或将路径写入 .vscode/debug.env 中的 AI_MANAGER_VSCODE_EXECUTABLE。');
  return discovered;
}
